  ...
  useEffect(()=>{
    async function fetchUser(){
      ...
      if(!user){
        ...
        setUserToken(userToken.data.listUserBs.items[0].token);
        setUserId(userToken.data.listUserBs.items[0].id)
      }
      return ;
    }
    fetchUser();
      ...
      onFormSubmit={async event => {
        ...
        {   
            ...
            if(response.err==null){
              ...
              await API.graphql(graphqlOperation(mutations.updateUserB, {input:{id:userId, token : tokenNum+userToken}})).then((res)=>{
                // console.log(res)
                setLoading(false);
                onClose();
                results();
